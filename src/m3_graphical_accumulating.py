...
def draw_parallel_lines(n, point, length, window):
    ...
    for k in range(n):
        p1 = rg.Point(point.x,point.y + (k*30))
        p2 = rg.Point(point.x + length, point.y + (k*30))
        theLine = rg.Line(p1,p2)
        print(theLine)
        theLine.attach_to(window)
    window.render()
...
def draw_lines(n, point, window):
    ...
    for k in range(n):
        ...
        p2 = rg.Point(point.x + 100, point.y - 100 + ((200*k)/(n-1)))
        theLine = rg.Line(p1,p2)
        print(theLine)
        theLine.attach_to(window)
    window.render()
