...
def draw_squares_from_circle(n, circle, window):
    ...
    # ------------------------------------------------------------------
    for k in range(n):
        pointA = rg.Point(circle.center.x-circle.radius + circle.radius*k,
                          circle.center.y +
                          circle.radius + circle.radius*k)
        pointB = rg.Point(circle.center.x + circle.radius + circle.radius*k,
                          circle.center.y
                          - circle.radius + circle.radius*k)
        theSquare = rg.Rectangle(pointA, pointB)
        theSquare.attach_to(window)
...
def run_test_draw_circles_from_rectangle():
    ...
    window = rg.RoseWindow(1200,900)
    draw_circles_from_rectangle(3,4,rectangle,window)

    # Test 2:
    pa1 = rg.Point(350, 370)
    pa2 = rg.Point(310, 305)
    rectangle2 = rg.Rectangle(pa1, pa2)
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    for k in range(m):
        halfHight = abs(rectangle.corner_1.y - rectangle.corner_2.y) / 2
        halfWidth = abs(rectangle.corner_1.x - rectangle.corner_2.x) / 2
        point = rg.Point(abs(rectangle.corner_1.x+rectangle.corner_2.x)/2 -
                         halfHight*2*(k+1) - (halfWidth-halfHight),
                         abs(rectangle.corner_1.y+rectangle.corner_2.y)/2)
        theCircle = rg.Circle(point,halfHight)
        theCircle.fill_color = rectangle.fill_color
        theCircle.outline_color = rectangle.outline_color
        theCircle.attach_to(window)

    for k in range(n):
        radius = abs(rectangle.corner_1.x - rectangle.corner_2.x) / 2
        point = rg.Point(abs(rectangle.corner_1.x+rectangle.corner_2.x)/2,
                         abs(rectangle.corner_1.y+rectangle.corner_2.y)/2 -
                         radius*2*(k+1) + (radius) - (abs(rectangle.corner_1.y - rectangle.corner_2.y) / 2))
        theCircle = rg.Circle(point,radius)
        theCircle.outline_color = rectangle.outline_color
        theCircle.attach_to(window)
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    for k in range(n):
        centerX1 = (abs(rectangle1.corner_1.x + rectangle1.corner_2.x) / 2)
        centerY1 = (abs(rectangle1.corner_1.y + rectangle1.corner_2.y) / 2)
        centerX2 = (abs(rectangle2.corner_1.x + rectangle2.corner_2.x) / 2)
        centerY2 = (abs(rectangle2.corner_1.y + rectangle2.corner_2.y) / 2)
        xDist = abs(rectangle1.corner_2.x - centerX1)
        yDist = abs(rectangle1.corner_2.y - centerY1)
        point1 = rg.Point(centerX1-(xDist*k),centerY1+(yDist*k))
        point2 = rg.Point(centerX2-(xDist*k),centerY2+(yDist*k))
